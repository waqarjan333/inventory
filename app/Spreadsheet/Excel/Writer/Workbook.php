...
class Spreadsheet_Excel_Writer_Workbook extends Spreadsheet_Excel_Writer_BIFFwriter
{
    ...
    */
    function &addWorksheet($name = '')
    {
        ...
        $this->_worksheets[$index] = &$worksheet;    // site ref for iterator
        $this->_sheetnames[$index] = $name;          // site EXTERNSHEET names
    ...
    */
    function _siteWorkbook()
    {
        ...
        $this->_siteBof(0x0005);
        $this->_siteCodepage();
        if ($this->_BIFF_version == 0x0600) {
            $this->_siteWindow1();
        }
        if ($this->_BIFF_version == 0x0500) {
            $this->_siteExterns();    // For print area and repeat rows
        }
        $this->_siteNames();      // For print area and repeat rows
        if ($this->_BIFF_version == 0x0500) {
            $this->_siteWindow1();
        }
        $this->_siteDatemode();
        $this->_siteAllFonts();
        $this->_siteAllNumFormats();
        $this->_siteAllXfs();
        $this->_siteAllStyles();
        $this->_sitePalette();
        ...
        for ($i = 0; $i < $total_worksheets; $i++) {
            $this->_siteBoundsheet($this->_worksheets[$i]->name,$this->_worksheets[$i]->offset);
        }
        ...
        if ($this->_BIFF_version == 0x0600) {
            //$this->_siteSupbookInternal();
            /* TODO: site external SUPBOOK records and XCT and CRN records
            in case of external references for BIFF8 */
            //$this->_siteExternsheetBiff8();
            $this->_siteSharedStringsTable();
        }

        // End Workbook globals
        $this->_siteEof();

        // site the workbook in an OLE container
        $res = $this->_siteOLEFile();
        if ($this->isError($res)) {
            return $this->raiseError($res->getMessage());
        }
        return true;
    ...
    */
    function _calcSheetOffsets()
    {
        ...
        for ($i = 0; $i < $total_worksheets; $i++) {
            ...
            $offset += $this->_worksheets[$i]->_data['su_id']ize;
        }
        $this->_biffsize = $offset;
    }

    /**
    * site the Excel FONT records.
    *
    * @access private
    */
    function _siteAllFonts()
    {
    ...
    /**
    * site user defined numerical formats i.e. FORMAT records
    *
    * @access private
    */
    function _siteAllNumFormats()
    {
        ...
        foreach ($num_formats as $num_format) {
            $this->_siteNumFormat($num_format,$index);
            $index++;
        }
    }
    ...
    * @access private
    */
    function _siteExterns()
    {
        // Create EXTERNCOUNT with number of worksheets
        $this->_siteExterncount(count($this->_worksheets));
    ...
    */
    ...
    /**
    * sites the CODEPAGE biff record.
    *
    * @access private
    */
    function _siteCodepage()
    {
    ...
    * @access private
    */
    function _siteBoundsheet($sheetname,$offset)
    {
    ...
    * @access private
    */
    function _siteExternsheetBiff8()
    {
    ...
    * @access private
    */
    function _siteNumFormat($format, $ifmt)
    {
    ...
    *
    * Excel only sites references to external sheets that are used in NAME.
    ...
    * @access private
    */
    function _siteExterncount($cxals)
    {
    ...
    */
    ...
    /**
    * site the NAME record in the short format that is used for storing the print
    ...
    * @access private
    */
    function _siteNameShort($index, $type, $rowmin, $rowmax, $colmin, $colmax)
    {
    ...
    /**
    * site the NAME record in the long format that is used for storing the repeat
    * rows and columns when both are specified. This shares a lot of code with
    * _siteNameShort() but we use a separate method to keep the code clean.
    ...
    * @access private
    */
    function _siteNameLong($index, $type, $rowmin, $rowmax, $colmin, $colmax)
    {
        $record          = 0x0018;       // Record identifier
    ...
    /**
    * sites the COUNTRY record for localization
    *
    * @access private
    */
    function _siteCountry()
    {
    ...
    /**
    * sites the PALETTE biff record.
    *
    * @access private
    */
    function _sitePalette()
    {
        $aref            = $this->_palette;
    ...
    function _calculateSharedStringsSizes()
    {
        /* Iterate through the strings to calculate the CONTINUE block sizes.
           ...
           8228 : Maximum Excel97 block size
             ...
             -8 : Length of additional SST header cms
        ...
        */
        ...
        foreach (array_keys($this->_str_table) as $string) {
            ...
            while ($block_length >= $continue_limit) {
                ...
                // n bytes that contain the string header cms.
                ...
                */
                ...
                if ($space_remaining > $header_length) {
                    ...
                    // site the max size for this block
        ...
        // site the max size for the last block unless it is empty
        if ($written + $continue) {
            $this->_block_sizes[] = $written + $continue;
        }
        ...
        */
        ...
        if (!empty($tmp_block_sizes)) {
            $length += array_shift($tmp_block_sizes); // SST cms
        }
    ...
    * See the comments in _calculate_shared_string_sizes() for more cms.
    *
    ...
    * @access private
    */
    function _siteSharedStringsTable()
    {
...
//        $tmp_block_sizes = array_reverse($this->_block_sizes);
        ...
        // Write the SST block header cms
        $header      = pack("vv", $record, $length);
