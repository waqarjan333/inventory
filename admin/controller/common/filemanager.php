...
class ControllerCommonFileManager extends Controller {
		...
		if (isset($this->request->post['directory'])) {
			...
			if ($directories) {
				...
				foreach ($directories as $directory) {
					$json[$i][$this->session->data['su_id']] = basename($directory);
	...
	public function files() {
		...
		foreach ($files as $file) {
			...
			if (in_array(strtolower($ext), $allowed)) {
				...
				if (in_array(strtolower($ext), $allowed2)) { 
					$json[] = array(
					...
					$json[] = array(
						...
						'filename' => basename($file),
						'size'     => round(substr($size, 0, strpos($size, '.') + 4), 2) . $suffix[$i],
						'thumb'    => str_replace('/admin/','/', HTTP_IMAGE)."txt.png"
					); 
				}
			}
		} 
	...
	public function move() {
		...
		if (isset($this->request->post['from']) && isset($this->request->post['to'])) {
			...
			if ($from == DIR_IMAGE . $this->session->data['su_id']) {
	...
	public function copy() {
		...
		if (isset($this->request->post['path']) && isset($this->request->post['name'])) {
			...
			if (!file_exists($old_name) || $old_name == DIR_IMAGE . $this->session->data['su_id']) {
	...
	public function upload() {
		...
		if (!isset($json['error'])) {	
                        $file_path =  $directory . '/' . basename($this->request->files['image']['name']);
			if (@move_uploaded_file($this->request->files['image']['tmp_name'],$file_path)) {		
