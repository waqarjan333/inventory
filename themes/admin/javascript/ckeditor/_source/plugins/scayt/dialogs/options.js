...
CKEDITOR.dialog.add( 'scaytcheck', function( editor )
{
					...
					elements : [
						{
							...
							html : 	'<div class="inner_dictionary" style="text-align:left; white-space:normal;">' +
									...
									'				</a>' +
									'			<a  style="display:none;" class="cke_dialog_ui_button" href="javascript:void(0)" id="dic_resite">'+
									'				</a>' +
									'		</div>' +
	...
	var init_with_captions = function()
	{
		var dialog = this,
			...
			buttons = [ 'dic_create','dic_delete','dic_rename','dic_resite' ],
		...
		dic.dic_resite = dialog.dic_resite || function ( el, dic_name , dic_buttons )
			{
				// try to resite existing dictionary
				var all_buttons = dic_buttons[0] + ',' + dic_buttons[1];
				var err_massage = captions["err_dic_resite"];
				var suc_massage = captions["succ_dic_resite"];

				window.scayt.resiteUserDictionary(dic_name,
					function(arg)
						{
							//console.info( "dic_resite callback called with args" , arg );
							suc_massage = suc_massage.replace("%s" , arg.dname );
							hide_dic_buttons ( all_buttons );
							display_dic_buttons(dic_buttons[1]);
							dic_success_message( suc_massage );
					...
					function(arg)
						{
							//console.info( " dic_resite errorback called with args" , arg )
							err_massage = err_massage.replace("%s" , arg.dname );
							dic_error_message( err_massage );
						});
			};
