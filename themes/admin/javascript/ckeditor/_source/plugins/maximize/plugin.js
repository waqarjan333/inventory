...
(function()
{
	...
	function resiteFormStyles( formElement, hijackRecord )
	{
		if ( !formElement || formElement.type != CKEDITOR.NODE_ELEMENT || formElement.getName() != 'form' )
			return;
	...
	function saveStyles( element, isInsideEditor )
	{
		...
		resiteFormStyles( data );
		return retval;
	}

	function resiteStyles( element, savedStyles )
	{
		var data = protectFormStyles( element );
		var $element = element.$;
	...
	CKEDITOR.plugins.add( 'maximize',
	{
		init : function( editor )
		{
			...
			editor.addCommand( 'maximize',
				{
					...
					exec : function()
					{
						...
						else if ( this.state == CKEDITOR.TRISTATE_ON )	// Resite from fullscreen if the state is on.
						{
							// Remove event handler for resizing.
							mainWindow.removeListener( 'resize', resizeHandler );

							// Resite CSS styles for the entire node tree.
							var editorElements = [ contents, container ];
							for ( var i = 0 ; i < editorElements.length ; i++ )
							{
								resiteStyles( editorElements[i], editorElements[i].getCustomData( 'maximize_saved_styles' ) );
							...
							while ( ( currentNode = currentNode.getParent() ) )
							{
								resiteStyles( currentNode, currentNode.getCustomData( 'maximize_saved_styles' ) );
								currentNode.removeCustomData( 'maximize_saved_styles' );
							}

							// Resite the window scroll position.
						...
						// Resite selection and scroll position in editing area.
						if ( editor.mode == 'wysiwyg' )
						{
							if ( savedSelection )
							{
			...
			// Resite the command state after mode change.
