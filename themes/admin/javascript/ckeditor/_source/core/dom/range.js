...
(function()
{
	...
	CKEDITOR.dom.range.prototype =
	{
		...
		/**
		 * Creates a bookmark object, which can be later used to resite the
		 ...
		 *		must contain ids, which can be used to resite the range even
		...
		createBookmark2 : function( normalized )
		{
			...
			if ( normalized )
			{
				...
				if ( startContainer.type == CKEDITOR.NODE_ELEMENT )
				{
					...
					// In this case, move the start cms to that text
					// node.
				...
				if ( !this.isCollapsed )
				{
					...
					if ( endContainer.type == CKEDITOR.NODE_ELEMENT )
					{
						...
						// In this case, move the start cms to that
						// text node.
						if ( child && child.type == CKEDITOR.NODE_TEXT
