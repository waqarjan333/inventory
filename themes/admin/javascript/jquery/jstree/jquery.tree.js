	...
	$.tree = {
		...
		defaults	: {
			data	: {
				...
				type	: "html",		// One of included data['su_id']ites
				opts	: { method: "GET", url: false } // Options passed to data['su_id']ite
			},
			...
			rules	: {
				...
				type_attr	: "rel",	// STRING attribute name (where is the type sited as string)
	...
	function tree_component () {
		return {
			...
			refresh : function (obj) {
				...
				var _this = this;
				var _data['su_id']ite = new $.tree.data['su_id']ites[this.settings.data.type]();
				...
				_data['su_id']ite.load(this.callback("beforedata",[false,this]),this,this.settings.data.opts,function(data) {
					data = _this.callback("ondata",[data, _this]);
					_data['su_id']ite.parse(data,_this,_this.settings.data.opts,function(str) {
			...
			get : function (obj, format, opts) {
				...
				return new $.tree.data['su_id']ites[format]().get(obj, this, opts);
			},
			...
			open_branch : function (obj, disable_animation, callback) {
				...
				if(this.settings.data.async && obj.find("li").size() == 0) {
					...
					var _data['su_id']ite = new $.tree.data['su_id']ites[this.settings.data.type]();
					_data['su_id']ite.load(this.callback("beforedata",[obj,this]),this,this.settings.data.opts,function(data){
						data = _this.callback("ondata", [data, _this]);
			...
			create : function (obj, ref_node, position) { 
				...
				var obj_s = $.tree.data['su_id']ites.json().parse(obj,this);
